...
int findMax(PAddress phead, PAddress plast)
{
	int max = phead->x;
	for (PAddress node = phead; node != NULL; node = node->next)
	{
		if (node->x > max)
			max = node->x;
	}
...
int main(void)
{
	...
	while (true)
	{
		...
		case 6:  readFromFile(&head, &last);
			break;
		case 7:	int k;
			cout << "Введите число удаляемых элементов с конца: ";
			cin >> k;
			cin.ignore(cin.rdbuf()->in_avail());
			cin.sync();
			deletk(k, &head, &last);
			break;
		case 8: cout << findMax(head, last);
		case 9:  exit(0);
		default: exit(1);
		}
	}
